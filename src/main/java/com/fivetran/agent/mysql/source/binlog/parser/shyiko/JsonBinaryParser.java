...
public class JsonBinaryParser {
    ...
    public static void parse(byte[] bytes, JsonFormatter formatter) throws IOException {
        new JsonBinaryParser(bytes).parse(formatter);
    }

    private final BinlogInputStream reader;

    public JsonBinaryParser(byte[] bytes) {
        this(new BinlogInputStream(bytes));
    }

    public JsonBinaryParser(BinlogInputStream contents) {
    ...
    protected void parseObject(boolean small, JsonFormatter formatter)
        ...
        ValueEntry[] entries = parseJsonEntries(numElements, numBytes, small);
    ...
    protected ValueEntry[] parseJsonEntries(int numElements, int numBytes, boolean small)
            throws IOException {
        // Read each key value value-entry
    ...
    protected void parseDecimal(int length, JsonFormatter formatter) throws IOException {
        ...
        BigDecimal dec = RowParser.asBigDecimal(precision, scale, reader.read(decimalLength));
        formatter.value(dec);
    }
